...
export const MemoizationPage = () => {
  ...
  const [users, setUsers] = useState(() => generateUsers(5000000));
  ...
  return (
    <div>
      ...
      <UserList users={sortedUsers} onSelect={onSelect} />
    </div>
  );
};
...
const UserItem = memo(
  ({ user, onSelect }: { user: User; onSelect: (id: number) => void }) => {
    // eslint-disable-next-line react-hooks/purity
    const id = Math.random();
    return (
      <li>
